...
`freppledb.mlcc.solver` 将 frePPLe 基础数据和 MLCC 扩展表转换为求解器中立的 `PlanningInstance`。数据库访问仅存在于 `extractor.py`；`schema.py`、`validator.py` 和 `serializer.py` 不接收或返回 Django ORM 对象。本阶段不引入 OR-Tools，也不生成正式排产结果。
...
- `schema_version` 当前为 `mlcc-planning-instance/v1`。
...
### Recipe、CompatibilityRule 与 SetupRule
...
| 能力/配方/同炉/换型 | `MlccEquipmentCapability`、`MlccRecipe`、`MlccCompatibilityRule`、`MlccSetupMatrix` |
