"""Pure CP-SAT finite-capacity baseline consuming only PlanningInstance data."""
...
def _priority_weights(orders):
    ...
    return {
        order.id: max(1, maximum + 1 - max(1, order.priority))
        for order in orders
    }
...
def _build_model(instance):
    ...
    for step in steps:
        for predecessor_id in step.predecessor_ids:
            ...
            model.Add(
                starts[step.id]
                >= ends[predecessor_id] + step.minimum_wait_minutes
            )
            if step.maximum_wait_minutes is not None:
                model.Add(
                    starts[step.id]
                    <= ends[predecessor_id] + step.maximum_wait_minutes
                )
...
def solve(instance, parameters=None):
    ...
    stages.append(
        _stage_result("makespan", makespan_solver, status, makespan_value)
    )
    if status in (cp_model.FEASIBLE, cp_model.OPTIMAL):
